 ...
 * 【解】： A[]={1,2,3}，求B[]
    ...
    3. C[i]计算完毕求 D[i],设置一个临时变量 org.tianbin.temp 初始化为 1
    4. 从后往前变量数组，LengthA=3 初始化 i=LengthA-2=1，结束条件为 i>=0
    5. 第一次循环，org.tianbin.temp=org.tianbin.temp×A[i+1]=1×A[2]=3，计算出A中最后一个元素的值放入 org.tianbin.temp，org.tianbin.temp 相当于 D[i]的值
    6. 因为之前的 B[i]=C[i]，所以让 B[i]×D[i] 就是要保存的结果，即 B[i]=B[1]=B[1]×org.tianbin.temp=1×3=3,i–=0
    7. 计算 B[i]=B[0]，temp上一步中的值是A[2]，在这次循环中 org.tianbin.temp=org.tianbin.temp×A[0+1]=A[2]×A[1]=3×2=6
    8， B[i]=B[0]=B0]×org.tianbin.temp=1×6=6，i–<0循 环结束
