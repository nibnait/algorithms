...
//        System.out.println(difference1);
    ...
    public void testSubList() {
        List<Integer> imgList = Lists.newArrayList(1, 2, 3, 4, 5);
        List<Integer> subImgList = imgList.subList(1, imgList.size());
        Assert.assertTrue(CompareUtils.match(subImgList, Lists.newArrayList(2,3,4,5)));

        imgList = Lists.newArrayList(1);
        subImgList = imgList.subList(1, imgList.size());
        Assert.assertTrue(CompareUtils.match(subImgList, Lists.newArrayList()));
         ...
         * subList 是 左闭右开区间
         */
        List<Integer> result = list.subList(0, 2);
        List<Integer> expect = Lists.newArrayList(0, 1);
        Assert.assertTrue(CompareUtils.match(result, expect));

        result = list.subList(1, 2);
        expect = Lists.newArrayList(1);
        Assert.assertTrue(CompareUtils.match(result, expect));
