package Algorithm.Coding_Interview_Guide_2ndEdition.Chapter_01_栈和队列;
...
public class P11_可见的山峰对数量 extends TestCase {
    ...
    /* pop为倒数第2组节点
       ...
       如果peek只有1个，则pop这组节点 还要加上pop.times，（getCurrentNodeNum只计算了从next方向上计算的数量）
                                         （从last方向上看的组合）
    ...
    /**
     * 1. 转化为单调栈结构
     * 因为矮的山峰之间如果隔了一个高的山峰，就互相看不到了，因此此题可转化为：【寻找离每个节点最近的比它大的元素的index】
     * 即 需要栈顶到栈底依次递增
     * <p>
    ...
    /*2. 计算当前节点的可见山峰数
      当前弹出的节点
      times == 1，说明pop next方向的邻节点（遍历到当前的节点），比pop.value大
                     pop last方向的邻节点（stack.peek）也比pop.value大
                  则此节点的可见山峰数为2
      times > 1，说明有多个与pop.value相同的节点，这几个pop节点之间 比pop.value小的节点已经被弹出，并将自己的可见山峰数计算完毕
                下面只需计算这几个value相同的节点的可见山峰数即可，假设有k个【k = pop.getTimes()】
