...
则中位数是 (2 + 3)/2 = 2.5
 */
public class E004_寻找两个有序数组的中位数 extends TestCase {
...
//        int[] nums2 = new int[]{3, 4};
    ...
    private double getMedian(int iMin, int iMax) {
        ...
        if (i < iMax && A[i] < B[j - 1]) {
            ...
            double maxLeft = 0;
            if (i == 0) maxLeft = B[j - 1]; //此时A[i-1]不存在
            else if (j == 0) maxLeft = A[i - 1];  //此时B[j-1]不存在
            ...
            double minRight = 0;
            if (i == m) minRight = B[j];    //此时A[i] 不存在
            else if (j == n) minRight = A[i]; //此时B[j] 不存在
